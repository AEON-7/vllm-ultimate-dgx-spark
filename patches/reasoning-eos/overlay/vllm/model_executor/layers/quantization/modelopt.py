...
QUANT_ALGOS = [
    ...
    "W4A16_NVFP4",
    # NVFP4 with AWQ pre-quantization scaling (ModelOpt 0.42.x). Same tensor
    # layout as NVFP4 plus optional per-linear `pre_quant_scale` vectors.
    "NVFP4_AWQ",
    # MXFP8
    "MXFP8",
    # MIXED_PRECISION,
...
class ModelOptNvFp4Config(ModelOptQuantConfigBase):
    ...
    def __init__(
        ...
        # aeon_gemma31_nvfp4_awq: NVFP4_AWQ is W4A4 NVFP4 produced by AWQ
        # calibration -- identical tensor layout plus optional per-linear
        # `pre_quant_scale` vectors -- so it uses the same LinearMethod. This
        # dispatch is NEW in v0.26.0 (added with W4A16_NVFP4) and is a SECOND
        # gate beyond the QUANT_ALGOS list.
...
class ModelOptNvFp4LinearMethod(LinearMethodBase):
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        # aeon_gemma31_nvfp4_awq: optional AWQ activation pre-scale. Present in ModelOpt NVFP4_AWQ
        # checkpoints on a SUBSET of linears (Gemma-4-31B: down_proj + o_proj;
        # E4B also per_layer_* and the KV-shared q_proj). Layers with no matching
        # checkpoint tensor keep all-ones scaling and are an exact no-op. Sized
        # to input_size_per_partition because the vector is per-INPUT-channel --
        # the correct shard for RowParallel inputs, which is where every
        # Gemma-4-31B vector lives.
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        # aeon_gemma31_nvfp4_awq: scrub FP8 NaN block scales emitted by ModelOpt 0.42.x.
        # FP8 E4M3 NaN encodings are 0x7F and 0xFF. Zeroing the block scale is
        # preferable to propagating NaNs through the entire decode.
        ...
        # aeon_gemma31_nvfp4_awq: retain the AWQ activation pre-scale for runtime. Convert to a
        # plain Parameter so no BasevLLMParameter (which overrides
        # __torch_function__) becomes an operand inside torch.compile /
        # CUDA-graph capture.
        if hasattr(layer, "pre_quant_scale"):
            ...
            if torch.allclose(pqs, torch.ones_like(pqs)):
                ...
                logger.info(
                    "Loaded AWQ pre_quant_scale: shape=%s min=%.4f max=%.4f "
                    "mean=%.4f",
                    list(pqs.shape),
                    pqs.min().item(),
                    pqs.max().item(),
    ...
    def apply(
        ...
        # aeon_gemma31_nvfp4_awq: apply the ModelOpt AWQ activation pre-scale before the FP4
        # matmul. Weights were divided by this vector at quantization time
        # (y = (x*s) @ (W/s)); omitting it yields garbage, not a subtle
        # regression -- measured s has mean ~2.35, range [0.18, 5.47]. Sitting
        # above the kernel dispatch keeps this backend-agnostic (CUTLASS/Marlin).
...
class ModelOptMixedPrecisionConfig(ModelOptQuantConfigBase):
    ...
    def get_quant_method(
        ...
        if isinstance(layer, (LinearBase, ParallelLMHead)):
            ...
            if quant_algo in ("NVFP4", "NVFP4_AWQ"):
