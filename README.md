...
[![vLLM](https://img.shields.io/badge/vLLM-0.29.0%2Baeon.sm__121a.omni-orange)](https://github.com/vllm-project/vllm)
...
Built on **vLLM v0.29.0 compiled from source for sm_121a (see 0.29-omni release note)**, merged with the AEON speculative-decoding stack: **DSpark Markov-head speculation (quantized heads)** + Triton software **NVFP4 KV cache** (PR #44389, NHD-safe rewrite) + **DFlash** (SWA drafters native on Model-Runner-V2) + the **AEON DGX Spark runtime patches** + **TurboQuant**.

> 🆕 **2026-09-11 — vLLM 0.29.0-omni (`:2026-09-11-v0.29.0-omni`).** The Spark engine moves from **0.27.1** to **upstream v0.29.0**, still compiled for **sm_121a**, still carrying the AEON stack that makes GB10 behave: DFlash/DFlash2, NVFP4 paths, reasoning-EOS force-end, and the cross-node CUDA-graph carry. The headline for operators: **dual-Spark TP=2 stops salad-ing** when you turn **FlashInfer all-reduce off**, disable **fuse_allreduce_rms**, keep **`--disable-custom-all-reduce`**, and use a single-`=` `NCCL_IB_HCA=…:1` — 0.29 enables FlashInfer AR by default, and that default is wrong for RoCE Sparks until you prove otherwise. Single-Spark TP=1 A/B vs `2026-09-07-reasoning-eos` held quality and was slightly faster. **Pin the dated tag** (digest `sha256:2421bb1228a85370c1c50adb31f605c4361acf4d48d65282fcb919e74f34fae7`); MIXED still needs the ModelOpt `#54367`-style bind on both ranks; Omni may log a missing `aenum` until the next fold. Full notes: [docs/RELEASE-0.29.0-omni.md](docs/RELEASE-0.29.0-omni.md). Rollback: `:2026-09-07-reasoning-eos`.

> 🔊 **2026-08-24 — `:latest` is now the OMNI image (`:2026-08-24-v0.27.1-omni`).** One container that hears **and speaks**: `vllm-omni 0.27.0rc1` is layered on, so the Thinker→Talker→Code2Wav speech stages work. Stock vLLM cannot do this at all — its loaders strip the voice half (`skip_prefixes=["talker.", "code2wav."]`). This is also the **first image anywhere to ship DFlash 2** — upstream [#52816](https://github.com/vllm-project/vllm/pull/52816) merged *one day* after 0.27.1, cherry-picked here as a pure-Python layer: measured **3.39× single-stream** and **1.41× at 64 concurrent** on Qwen3.8-27B, beating MTP and DSpark at every concurrency level. Plus audio-in (PyAV/soundfile/librosa/soxr), the `mistral_common` fix, and **1M context** via official YaRN — needle-verified **3/3 at 543,761 tokens**. Full notes: [docs/RELEASE-0.27.1-omni.md](docs/RELEASE-0.27.1-omni.md).

> ✅ **Correction (2026-08-24) — the NCCL 2.30.7 credit was wrong.** Cross-node CUDA graph stability was previously attributed to NCCL 2.30.7, with a stated floor of 2.30.4. Both shipping images run **NCCL 2.29.7** and cross-node capture *and* replay held for 5+ hours. The load-bearing carry is the [#48053](https://github.com/vllm-project/vllm/pull/48053) `thread_local` capture-error-mode — **there is no 2.30.x floor.** TP=2 is also no longer "unvalidated": validated over RoCE v2 with RDMA proven by *counters* rather than inferred from logs — a 400-token generation moved **+3,210 MB** through `port_xmit_data` while netdev `tx_bytes` moved **0 MB**.

> 🆕 **2026-08-17 — `:latest` is now the SLIM image: 18.3 GB instead of 50.6 GB (~8.2 GB to pull, down from 21.4 GB).** Same vLLM 0.27.1 build, same userland, same gate results — just packaged without the dead weight. Two measured sources of fat: the inherited base carried a **vLLM-0.20.1-era stack buried under layer whiteouts** (the old image's *live* filesystem was only ~22 GB of its 50.6 GB), and ~3.8 GB of `/usr/local/cuda` was static archives plus **exact duplicates of the pip `nvidia-*` wheels** that torch never loads (proven via `/proc/self/maps`). Rebased onto `nvidia/cuda:13.0.2-base-ubuntu22.04`, carrying the runtime organs explicitly — `ptxas`/`nvcc`/`nvvm` (no CUDA base image ships them), rdma-core for the TP=2 RoCE path, and the unversioned `gcc`/`cc`/`make` aliases Triton shells out to at runtime. **Validated on GB10 across all four fleet phases** (Gemma DFlash, Qwen3.6 DFlash, DSpark Markov heads, NVFP4-KV) with acceptance lengths matching the fat image, plus a real serve with CUDA-graph capture. Build: [`Dockerfile.slim`](Dockerfile.slim). Rollback: `:2026-08-16-v0.27.1` (the 50.6 GB image, unchanged).

> ⚡ **2026-08-17 — cross-node CUDA graphs work on dual-Spark TP=2.** Upstream [#46253](https://github.com/vllm-project/vllm/issues/46253) says multi-node GB10 must run `--enforce-eager`; this image doesn't need to. Our rebased [#48053](https://github.com/vllm-project/vllm/pull/48053) carry sets `capture_error_mode="thread_local"` at **all 7** capture sites, and with NCCL **2.29.7** + allreduce fusion off + `--disable-custom-all-reduce`, capture *and* replay are stable: 64-request soak, 6-cycle burn-in, **0** NCCL errors. Worth **+26% single-stream** (33.7 vs 26.7 tok/s) and **−23% TPOT**. Recipe in [AGENTS.md](AGENTS.md); findings [reported upstream](https://github.com/vllm-project/vllm/issues/46253#issuecomment-5362138890).

> 🆕 **2026-08-16 — the vLLM 0.27.1 sm_121a build (`:2026-08-16-v0.27.1`) — the DSPARK release.** 3-way merge of upstream **v0.27.1** (`6e448d0ea9`, 577 commits; v0.27.1 = v0.27.0 + #50424) onto the AEON tree — only 7 conflicts — plus **8 post-tag cherry-picks**. **Headline: DSpark Markov-head speculative decoding with QUANTIZED heads** (#50424 + #49969 top-k projection) — pairs with the NVFP4 fleet bodies; drafters on the Hub for Qwen3.6-27B (`satgeze`) and 35B-A3B (`RedHatAI`); Gemma-4-26B stays DFlash until a drafter is trained. **The `VLLM_USE_V2_MODEL_RUNNER=0` pin is GONE**: DSpark is MRv2-only and mixed-SWA DFlash drafters (all z-lab fleet drafters) auto-route to MRv2, where upstream #47914/#48113 run drafter SWA natively — drop the env from launch scripts; re-pin per-service only for `thinking_token_budget`. Cherry-picks: #50276 packed-KV zeroer stride (correctness under our NHD packed NVFP4-KV), #51812 GDN gate/spec-token alignment (Qwen3.6+DFlash), #51843 hybrid-APC fix, DSpark set #51602/#50693/#52288/#50910. Carries re-verified: #44389 NVFP4-KV (NHD-safe slicing; upstream still `as_strided`-unsafe; build-gated), #41703, UMA clamp, V1-scoped cudagraph align; **#48053 thread_local capture extended to all 5 graph sites** (2×-Spark TP=2; and this is exactly what makes **cross-node CUDA graphs work on this image** — see the dual-Spark TP=2 recipe in AGENTS.md (+26% single-stream vs eager; upstream #46253 still says eager-only)). Dropped (now upstream): #43982-port, #50065, #49659. Deps: **torch 2.13.0+cu130 / Triton 3.7.1 / FlashInfer 0.6.16.post3 trio (prebuilt ARM jit-cache) / NCCL 2.29.7 / transformers 5.14.1 / torchcodec 0.16.0 (video decode is BACK)**. Rollback: `:2026-07-27-v0.26.0`.

> **2026-07-27 (historical) — vLLM 0.26.0 sm_121a build (`:2026-07-27-v0.26.0`).** 3-way merge of upstream **v0.26.0** (`568afb3a1`, 429 commits) onto the AEON tree. **Headline:** Gemma-4-26B-A4B hits **1151 tok/s at 128 concurrent** (+46% over the old 32-batch cap) with 0 errors — the old "cap `max-num-seqs` ≤32 with DFlash" guidance is retired. **NVFP4_AWQ checkpoints now load** (Gemma-4-31B-DECKARD: 120/120 `pre_quant_scale` vectors, 33.6 tok/s mean / 45.4 on code, best DFlash acceptance of the fleet at 4.64) — previously impossible on any vLLM. Carries re-verified: #44389 Triton NVFP4-KV (**re-derived** for #44455's 4-D packed layout using NHD-safe slicing — upstream's `as_strided` math silently corrupts KV on GB10), #40898 DFlash SWA on V1 (upstream v0.26.0 now *refuses* our mixed sliding/full drafters on V1), #41703, block-table unpad, cudagraph align, UMA clamp. New: #46253 TP=2 cudagraph fix, #50065 padded-batch draft buffers, #49659 MoE-gated warmup. Dropped (now upstream): our lm_head fix (#47914), #47053/#47356/#45207/#48330. Deps: **FlashInfer 0.6.14**, cutlass-dsl 4.6.0, quack-kernels 0.6.1, apache-tvm-ffi 0.1.10; **torch 2.11.0 unchanged**. ⚠️ **Do NOT set `VLLM_ENABLE_STARTUP_PLAN=1` on GB10** — NVML has no memory support on unified memory; the engine dies at init. `VLLM_USE_V2_MODEL_RUNNER=0` stays baked. TP=2 wired in but **UNTESTED** (no second Spark). Rollback: `:2026-07-16-v0.25.1`.

> ℹ️ **torchcodec is BACK as of 0.27.1** (0.16.0, stable-ABI, torch-2.13-compatible) — video decode works again; the 0.14-vs-torch-2.11 ABI exclusion is history. **NVFP4 backend note:** checkpoints with per-input global scales (e.g. Gemma-4-26B, Qwen3.6-27B) run native **CUTLASS** FP4; the Ornith-35B checkpoint lacks them so it correctly uses **Marlin** — forcing `--linear-backend flashinfer_cutlass` on it crashes (missing `input_global_scale_inv`); re-quantize with CUTLASS-compatible scales to unlock it.
...
```bash
# 1) Pull the unified container (vLLM 0.27.1 + sm_121a + DFlash/DSpark on MRv2 auto-routing)
...
> ⚠️ **Pick `num_speculative_tokens` against your concurrency target.** The drafter's KV footprint is what caps real concurrency, so a high `n` that wins single-stream can *lose* under load. Measured on Qwen3.8-27B at 16K/fp8-KV: DFlash **n=12 drops to 0.53× baseline at 64 concurrent** with a 22.6 s TTFT, because its KV pool falls to 328,590 tokens (engine max concurrency 20.1×) versus 1,521,371 (92.9×) with no drafter. Use high `n` for interactive single-stream, low `n` for batch. Full table in [docs/RELEASE-0.27.1-omni.md](docs/RELEASE-0.27.1-omni.md).
...
- Version string: `0.25.1+aeon.sm121a.dflash`. Validated before push: 27B DFlash on **MRv2** (mean-accept-len 2.84–4.36), 27B DFlash on **V1**, and the Gemma-26B voice recipe on V1 (accept 2.92 / 19.2% = prod norm). Rollback: `:2026-07-14-v0.25.0`.

### The 0.25.0 rebase underneath (2026-07-14)
...
- **FlashInfer 0.6.13** (from 0.6.12; *superseded — the 2026-07-27 build ships 0.6.14*) — FP8-KV FA2 prefill fix + sm12x gating + a Mamba SSU cross-warp race fix relevant to the 26B hybrid. Stale 0.6.12 JIT-cache purged so 0.6.13 kernels aren't shadowed.
...
|---|---|---|
| **vLLM** | 0.27.1 + sm_121a build, AEON spec-decode 3-way merge + 8 cherry-picks | Built from source for GB10; DSpark quantized Markov heads (#50424/#49969), carries #44389 (Triton NVFP4 KV, NHD-safe), #41703, UMA clamp, #48053 thread_local capture; DFlash SWA now upstream on MRv2 (#47914/#48113) |
...
| **flashinfer** | 0.6.16.post3 (python+cubin+jit-cache trio) | NVFP4 GEMM kernels, SWA, MLA; prebuilt aarch64 JIT cache — faster cold starts on GB10 |
...
```bash
docker pull ghcr.io/aeon-7/aeon-vllm-ultimate:latest   # slim, ~8.2 GB pull
# or pin the current build (vLLM 0.27.1 + sm_121a + DSpark quantized Markov heads)
docker pull ghcr.io/aeon-7/aeon-vllm-ultimate:2026-08-16-v0.27.1
